...
.then(function(respknse){

    //Parse "response" to display 5 day forecast underneath current weather conditions//
console.log(response);
const forecastEls.length; i++) {
    forecastEls[i].innerHTML = "";
    const forecastIndex = i*8 + 4;
    const forecstDate = new Date(response.data.list[forecastIndex].dt * 1000);
    ...
    forecastEls[i].setAttribute("alt", response.data.list[forecastIndex].weather[0].description);
...
searchEl.addEventListener("click", function(){
    const searchTern = inputEl.value;
